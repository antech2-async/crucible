      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-border bg-surface-low">
            {columns.map((col) => (
              <th
                key={col.key}
                className={cn('px-4 py-3 text-left text-[10px] font-mono uppercase tracking-widest text-on-surface-muted', col.className)}
              >
                {col.header}
              </th>
            ))}
        ...
        <tbody>
          {data.length === 0 ? (
            ...
            data.map((row) => (
              <tr key={keyExtractor(row)} className="border-b border-border last:border-0 hover:bg-surface-low transition-colors">
                {columns.map((col) => (
