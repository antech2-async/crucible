...
export default function AgentDossier() {
  ...
  const _tierLabel = TIER_LABELS[agent.tier] || 'Basic';
  const tierColor = TIER_COLORS[agent.tier] || 'text-slate-400';

  const formattedDate = agent.registrationTime
    ? new Date(agent.registrationTime * 1000).toLocaleDateString('en-US', {
        month: 'short',
        year: 'numeric',
      })
    : 'Apr 2026';
    ...
    <div className="w-full max-w-6xl mx-auto pb-24">
        ...
        <Surface level="container" className="p-6 flex flex-col justify-center">
          ...
          <p className="text-xs font-mono text-on-surface-muted leading-relaxed mb-5">
            ...
            <strong className="text-on-surface">
              {agent.capabilities?.join(', ') || 'Unknown'}
            </strong>
            <br />
            <br />
            Trust score of{' '}
      ...
      <div className="space-y-2">
        {taskHistory.length > 0 ? (
          ...
          <div className="text-on-surface-muted text-xs font-mono py-8 text-center border border-border bg-surface-container">
      ...
      <div className="flex items-center gap-4">
        <div
          className={cn(
            ...
            isWarning ? 'border-danger/30' : 'border-border',
          )}
        >
          {isWarning ? (
