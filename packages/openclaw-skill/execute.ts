...
async function main() {
  ...
  // 2. High-Fidelity Context Handoff (Spec Section 13)
  let context = "";
  if (isSequential && currentStage > 0) {
      ...
      // Simulate fetching full text from 0G Storage
      const uri = `0g://crucible/${prevOutputHash.slice(2, 12)}`;
      context = await storage.fetch(uri);
  ...
  const prompt = isSequential && currentStage === 0 
    ? `Perform in-depth research on: ${task.criteriaURI}. Focus on 0G ecosystem details.`
    : `Using this research context: ${context}, write a professional executive summary.`;
