...
contract TaskEscrow is ReentrancyGuard, Ownable {
  ...
  constructor(address _engine, address _vault, address _registry) Ownable(msg.sender) {
  ...
  function assignAgents(
    ...
    for (uint i = 0; i < agents.length; i++) {
      ...
      if (a.totalTasksCompleted == 0 && !vault.hasCompletedFirstTask(agents[i])) {
          vault.lockStakeWithSubsidy(a.owner, agents[i], stakes[i], taskId);
      } else {
  ...
  function submitOutput(
    ...
    Task storage t = tasks[taskId];
    if (t.status != TaskStatus.ASSIGNED && t.status != TaskStatus.IN_PIPELINE) revert ("Task not in submission state");
