...
contract AgentRegistry is Ownable, ReentrancyGuard {
    ...
    function _register(
        ...
        if (inftContract.ownerOf(inftTokenId) != msg.sender)
            revert NotINFTOwner();
        ...
        uint256 requiredStake = agentClass == AgentClass.EXTERNAL
            ? (baseStake * 150) / 100
            : baseStake;
    ...
    function updateHistoryAndTrust(
        ...
        agent.minStakeRequired = agent.agentClass == AgentClass.EXTERNAL
            ? (baseStake * 150) / 100
            : baseStake;
    ...
    function getAgentsByCapability(
        ...
        for (uint i = 0; i < agentList.length; i++) {
            if (
                agents[agentList[i]].isActive &&
                _hasCap(agentList[i], capability)
            ) count++;
        }
