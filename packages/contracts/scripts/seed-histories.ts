...
async function uploadHistory(indexer: Indexer, signer: ethers.Wallet, history: object): Promise<string> {
    let jsonString = JSON.stringify(history);
    // Pad to 256KB to ensure it meets minimum segment requirements on some nodes
    if (jsonString.length < 262144) {
        jsonString = jsonString.padEnd(262144, ' ');
    }
    ...
    const fee = ethers.parseUnits("0.001", "ether");
    console.log(`  Submitting with safety fee: ${ethers.formatEther(fee)} OG (Padded size: ${jsonString.length} bytes)`);

    try {
        const uploadPromise = indexer.upload(memData, process.env.OG_RPC_URL!, signer, undefined, undefined, { gasLimit: 2000000n });
    ...
    for (const persona of personas) {
        ...
        try {
            ...
            if (!agentData.isActive) {
                ...
                const inft = new ethers.Contract(inftAddress, [
                ...
                const mintTx = await inft.mintAgent(signer.address, "ipfs://crucible-identity", ethers.ZeroHash, { 
                    value: mintFee
                });
                ...
                await (await registry.registerNativeAgent(
                    addr,
                    mintedTokenId,
                    ethers.ZeroHash,
                    persona.caps
            ...
            await (await registry.updateHistoryAndTrust(
                addr,
                rootHash,
                persona.tier,
                false
