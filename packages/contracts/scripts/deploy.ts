...
try {
  // @ts-expect-error - Global fetch override for ethers v6
  global.fetch = undefined;
} catch (e) {
  // Ignore errors if fetch is already undefined
}
...
async function main() {
  ...
  // @ts-expect-error - Internal ethers property access
  ...
  const judge = await (
    await getFactory('SlashingJudge').deploy(
      await registry.getAddress(),
  ...
  const inft = await (
    await getFactory('CrucibleINFT').deploy(deployer.address, { gasPrice: 10000000000n })
  ).waitForDeployment();
  ...
  await (
    await (registry as any).addAuthorizedUpdater(await judge.getAddress(), {
      gasPrice: 10000000000n,
    })
  ).wait();
  await (
    await (registry as any).setINFTContract(await inft.getAddress(), { gasPrice: 10000000000n })
  ).wait();
  await (
    await (escrow as any).setSlashingJudge(await judge.getAddress(), { gasPrice: 10000000000n })
  ).wait();
  await (
    await (vault as any).setEscrowContract(await escrow.getAddress(), { gasPrice: 10000000000n })
  ).wait();

  await (
    await (judge as any).addAuthorizedCaller(deployer.address, { gasPrice: 10000000000n })
  ).wait();
  ...
  try {
    ...
    for (let i = 0; i < taskBatch; i++) {
      await (
        await (escrow as any).postTask(deadline, criteriaBytes32, criteriaURI, false, {
