...
async function main() {
  ...
  const registry = await ethers.getContractAt(
    'AgentRegistry',
    process.env.REGISTRY_ADDRESS || ethers.ZeroAddress,
    deployer,
  );
  const vault = await ethers.getContractAt(
    'AgentStakeVault',
    process.env.VAULT_ADDRESS || ethers.ZeroAddress,
    deployer,
  );
  const inft = await ethers.getContractAt(
    'CrucibleINFT',
    process.env.INFT_ADDRESS || ethers.ZeroAddress,
    deployer,
  );
  ...
  for (const p of personae) {
    ...
    if (p.class === 0) {
      const tx = await registry
        .connect(p.signer)
        .registerNativeAgent(p.signer.address, tokenId, bytes32Hash, p.caps, { gasLimit: 300000 });
      await tx.wait();
    } else {
      const tx = await registry
        .connect(p.signer)
    ...
    const vtx = await vault
      .connect(p.signer)
      .deposit({ value: ethers.parseEther('0.005'), gasLimit: 100000 });
    await vtx.wait();
