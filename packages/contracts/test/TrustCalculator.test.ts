...
describe('TrustCalculator', () => {
  ...
  describe('calculateTrustTier', () => {
    it('returns tier 0 for new agent with < 3 tasks', async () => {
      ...
      expect(await calculator.calculateTrustTier(1, 1, 1, 0)).to.equal(0);
      expect(await calculator.calculateTrustTier(0, 0, 0, 0)).to.equal(0);
    });
    ...
    it('drops tier when slash events accumulate', async () => {
      ...
      expect(await calculator.calculateTrustTier(10, 10, 10, 1)).to.equal(4);

      // 2 slashes = penalty 1000. final = 9000 -> Tier 3
      expect(await calculator.calculateTrustTier(10, 10, 10, 2)).to.equal(3);

      // 3 slashes = penalty 1500. final = 8500 -> Tier 3
      expect(await calculator.calculateTrustTier(10, 10, 10, 3)).to.equal(3);

      // 6 slashes = penalty 3000. final = 7000 -> Tier 2
      expect(await calculator.calculateTrustTier(10, 10, 10, 6)).to.equal(2);
      
      // 10 slashes = penalty 5000. final = 5000 -> Tier 1
      expect(await calculator.calculateTrustTier(10, 10, 10, 10)).to.equal(1);
      
      // 11 slashes = penalty 5500. final = 4500 -> Tier 0
      expect(await calculator.calculateTrustTier(10, 10, 10, 11)).to.equal(0);
    });
    ...
    it('calculates weighted score correctly (60% recent, 40% lifetime)', async () => {
      ...
      // Tier 0
      expect(await calculator.calculateTrustTier(100, 100, 0, 0)).to.equal(0);

      // Scenario: Poor lifetime but perfect recently
      // total = 100, honestly = 10 (lifetime = 10%)
      // recent = 10 (recent = 100%)
      // weighted = 0.6 * 100 + 0.4 * 10 = 60 + 4 = 64% (6400 bp)
      // Tier 1
      expect(await calculator.calculateTrustTier(100, 10, 10, 0)).to.equal(1);
    });
  });
