...
import { TaskEscrow, AgentStakeVault, AgentRegistry, TrustCalculator, SlashingJudge } from '../typechain-types';
...
describe('Deep-Spec Hardening', () => {
  ...
  beforeEach(async () => {
    [owner, poster, agent1, assignmentEngine, inspector] = await ethers.getSigners();
    ...
    judge = await JudgeFactory.deploy(await registry.getAddress(), await escrow.getAddress(), await calculator.getAddress());
  ...
  describe('Claim Pattern & Dispute Safety', () => {
    it('prevents agents from claiming payments during the dispute window', async () => {
      ...
      await escrow.connect(poster).postTask(
        ...
        ethers.encodeBytes32String('criteria'), 'uri', false,
        { value: ethers.parseEther('1.0') }
      );

      await vault.connect(agent1).deposit({ value: ethers.parseEther('1.0') });
      await escrow.connect(assignmentEngine).assignAgents(0, [agent1.address], [ethers.parseEther('0.1')]);
      await escrow.connect(agent1).submitOutput(0, "output1", "0x");

      // 2. Resolve Task
      await judge.connect(owner).judgeTask(0, [agent1.address], [true], [ethers.ZeroHash], [4, 4, 1, 0]);

      // 3. Agent tries to claim immediately
      await expect(escrow.connect(agent1).claimPayment(0))
  ...
  describe('Subsidy Accounting Flow', () => {
    ...
    it('burns subsidy and collects fee on slash', async () => {
        ...
        // Fail Task
        await judge.connect(owner).judgeTask(0, [agent1.address], [false], [ethers.ZeroHash], [4, 4, 1, 1]);
