...
describe('SlashingJudge', () => {
  ...
  beforeEach(async () => {
    ...
    judge = await Judge.deploy(await registry.getAddress(), await escrow.getAddress(), await calculator.getAddress());

    // Setup Auth
    await registry.addAuthorizedUpdater(await judge.getAddress());
    await escrow.setSlashingJudge(await judge.getAddress());
    await escrow.setAssignmentEngine(engine.address);

    // Register Agents
    await registry.connect(agent1).registerAgent(agent1.address, 1, 'hash1', []);
    await registry.connect(agent2).registerAgent(agent2.address, 2, 'hash2', []);
    await registry.connect(agent3).registerAgent(agent3.address, 3, 'hash3', []);
  });
  ...
  describe('Consensus Enforcement (M-of-N)', () => {
    beforeEach(async () => {
      ...
      await escrow.connect(owner).postTask(3600000000, ethers.encodeBytes32String('hash'), 'uri', { value: ethers.parseEther('3.0') });
      await escrow.connect(engine).assignAgents(
        0, 
    ...
    it('Majority Consensus: 2 Pass, 1 Fail (Bad Actor Slashed)', async () => {
      ...
      await expect(
        judge.judgeTask(0, agents, results, newHashes, data)
      ).to.emit(judge, 'JudgmentIssued').withArgs(0, agents, results)
       .and.to.emit(escrow, 'AgentSlashed').withArgs(0, agent3.address, ethers.parseEther('0.1')); // Agent 3 slashed

      const escrowTask = await escrow.tasks(0);
      expect(escrowTask.status).to.equal(3); // COMPLETED
    });
    ...
    it('Total Defection: 1 Pass, 2 Fail (All returned)', async () => {
      ...
      const escrowTask = await escrow.tasks(0);
      expect(escrowTask.status).to.equal(5); // FAILED
    });
  });
});
