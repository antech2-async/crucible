...
export class AgentRunner {
  ...
  registerLocalAgent(address: string, agentInstance: any) {
  ...
  startListening() {
    ...
    this.escrowContract.on('AgentsAssigned', async (taskId, assignedAgents: string[], stakes) => {
      ...
      for (const assignedAddr of assignedAgents) {
        ...
        if (localAgent) {
          logger.info(`🟢 Swarm Match! Local Agent ${assignedAddr} was assigned to Task ${taskId.toString()}`);
