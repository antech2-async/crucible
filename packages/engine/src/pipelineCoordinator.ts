...
export class PipelineCoordinator {
  ...
  async triggerNextStage(taskId: string, stage: number, nextAgentAddress: string): Promise<void> {
    logger.info(
      `Pipeline Stage Triggered: Task ${taskId}, Stage ${stage}, Agent ${nextAgentAddress}`,
    );
    ...
    try {
      ...
      if (stage > 0) {
        ...
        if (prevOutputHash && prevOutputHash !== '') {
          ...
          const download = await this.storageService.downloadJSON<unknown>(prevOutputHash);
      ...
      const stageInstruction =
        criteria.stages?.[stage] || `Execute task stage ${stage} for topic ${criteriaURI}`;
      const systemPrompt =
        criteria.systemPrompts?.[stage] || 'You are a specialized AI agent in a Crucible swarm.';
