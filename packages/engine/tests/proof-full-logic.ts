 ...
 */
async function main() {
  const [deployer, poster, alice, bob, judge] = await ethers.getSigners();
  ...
  const escrow = await Escrow.deploy(deployer.address, await vault.getAddress(), await registry.getAddress());
  ...
  await inft.connect(alice).mintAgent(alice.address, "https://alice.ai", hashA, "0x", { value: ethers.parseEther("0.001") });
  await registry.connect(alice).registerExternalAgent(alice.address, 1, hashA, ['research'], "https://alice.ai");
  ...
  await inft.connect(bob).mintAgent(bob.address, "https://bob.ai", hashB, "0x", { value: ethers.parseEther("0.001") });
  // AgentClass 1 = EXTERNAL
  await registry.connect(bob).registerExternalAgent(bob.address, 2, hashB, ['writing'], "https://bob.ai");
  ...
  // Post
  const ptx1 = await escrow.connect(poster).postTask(deadline1, ethers.ZeroHash, "0g://task1", false, { value: payment1 });
  await ptx1.wait();
  ...
  if (balAfterA > balBeforeA) {
      console.log(`    ✅ Success: Alice was paid. Income: ${ethers.formatEther(balAfterA - balBeforeA)} ETH`);
  }
  ...
  const ptx2 = await escrow.connect(poster).postTask(deadline2, ethers.ZeroHash, "0g://task2", false, { value: payment2 });
  await ptx2.wait();
  ...
  if (vaultBeforeSlashB - vaultAfterSlashB === stakeB) {
      console.log(`    ✅ Success: Bob was Slashed ${ethers.formatEther(stakeB)} ETH for inactivity.`);
  }
  if (posterAfterRefund > posterBeforeRefund) {
      const surplus = posterAfterRefund - (posterBeforeRefund + payment2);
      if (surplus > 0n) {
          console.log(`    ✅ OCD Success: Poster received Insurance payout! Surplus: ${ethers.formatEther(surplus)} ETH`);
      } else {
  ...
  await inft.connect(charlie).mintAgent(charlie.address, "https://charlie.ai", hashC, "0x", { value: ethers.parseEther("0.001") });
  await registry.connect(charlie).registerExternalAgent(charlie.address, 3, hashC, ['research'], "https://charlie.ai");
  ...
  const ptx5 = await escrow.connect(poster).postTask(deadline5, ethers.ZeroHash, "0g://task5", false, { value: payment5 });
  await ptx5.wait();
  const taskId5 = currentTaskId++;

  const stakeC = ethers.parseEther('0.05'); // 50% should be subsidized
  
  const vaultBeforeLockC = await vault.deposits(charlie.address);
  await escrow.connect(deployer).assignAgents(taskId5, [charlie.address], [stakeC]);
  const vaultAfterLockC = await vault.deposits(charlie.address);
  ...
  if (subsidyAmount > 0n) {
      console.log(`    ✅ Success: Protocol subsidized ${ethers.formatEther(subsidyAmount)} OG for Charlie's first task.`);
  }
  ...
  const ptx3 = await escrow.connect(poster).postTask(deadline3, ethers.ZeroHash, "0g://task3", true, { value: payment3 });
  await ptx3.wait();
  const taskId3 = currentTaskId++;

  await escrow.connect(deployer).assignAgents(taskId3, [alice.address, bob.address], [stakeA, stakeB]);
  ...
  if (posterAfterPartial > posterBeforePartial) {
      ...
      console.log(`    ✅ Success: Poster received Partial Refund (${ethers.formatEther(refund)} ETH).`);
  }
  ...
  const ptx4 = await escrow.connect(poster).postTask(deadline4, ethers.ZeroHash, "0g://task4", false, { value: payment4 });
  await ptx4.wait();
  ...
  if (taskState.status === 6n) { // 6 = DISPUTED
