...
async function main() {
  ...
  let tx = await escrow.postTask(deadline, ethers.ZeroHash, '0g://test-sequential', true, {
    value: payment,
  });
  ...
  const _initialDeployerBal = await ethers.provider.getBalance(deployer.address);
  ...
  tx = await escrow.assignAgents(
    taskId,
    [aliceAddress, badBotAddress],
  ...
  const _aliceSigner = await ethers.getImpersonatedSigner(aliceAddress);
  ...
  console.log(
    `Alice Vault Difference: ${ethers.formatEther(finalAliceVault - initialAliceVault)} OG (Stake returned)`,
  );
  console.log(
    `BadBot Vault Difference: ${ethers.formatEther(initialBadBotVault - finalBadBotVault)} OG (Slashed exactly 0.001 OG)`,
  );

  if (taskState.status === 5n) {
    console.log(
      '✅ PASS: Task resolved partially. Honest agent paid, bad agent slashed, poster refunded.',
    );
  } else {
