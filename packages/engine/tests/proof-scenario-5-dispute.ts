...
async function main() {
  ...
  const _vaultAddress = process.env.VAULT_ADDRESS || ethers.ZeroAddress;
  ...
  let tx = await escrow.postTask(deadline, ethers.ZeroHash, '0g://test-dispute', false, {
    value: payment,
  });
  ...
  const qtx = await escrow.postTask(deadline, ethers.ZeroHash, '0g://test-dispute', true, {
    value: payment,
  });
  const qreceipt = await qtx.wait();
  const qtaskId = qreceipt!.logs
    .map((log) => {
      try {
        return escrow.interface.parseLog(log);
      } catch {
